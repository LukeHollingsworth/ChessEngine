...
int CheckBoard(const S_BOARD *pos) {   
	...
	for(t_piece = wP; t_piece <= bK; ++t_piece) {
		for(t_pce_num = 0; t_pce_num < pos->pceNum[t_piece]; ++t_pce_num) {
			...
			ASSERT(pos->pieces[sq120] == t_piece);	// Check pieces same as temporary
		}	
	}
	
	// check piece count and other counters	
	for(sq64 = 0; sq64 < 64; ++sq64) {// When a move is made, it's updated incrimentally - becomes long and complex for long sequences
		sq120 = SQ120(sq64);
	...
	for(t_piece = wP; t_piece <= bK; ++t_piece) {	// Checking nothing has gone wrong and that the positions match
